...
Redis是单线程的，所有的读写操作都是在一条主线程中完成。在高并发场景下，这条线程就变成了它的生命线。如果阻塞，都是噩梦。

### 7.1 发生阻塞

阻塞时，应用方会受到大量Redis超时异常。需要详细记录哪个Redis集群报警，是集群报警，还是集群中某个节点。
...
slowlog get {n} 来查看最近几条慢查询日志，但是慢查询只记录了Redis服务端的响应时间。

解决方案：

```text
1、修改为低算法度的命令，比如hgetall 改为hmget, 禁用keys、sort 等命令。
2、调整大对象：缩减大对象数据或者把大对象拆分为多个小对象，防止一次命令操作过多的数据。
如何发现大对象？Redis 有工具，可以通过scan 来查看。
```
...
（1）进程竞争，和其他CPU密集型服务部署在一起，容易进程竞争

（2）绑定CPU：

2、内存交换：

把内存换出到磁盘上，导致Redis性能急剧下降。
...
是指由于单个网卡队列只能使用一个CPU，高并发下网卡数据交换都集中在同一个CPU，导致无法充分利用多核CPU的情况。
